...
exports.getReceived = (userId) =>
  db.query(
    "SELECT r.* FROM registrations_streams AS rs JOIN registrations ON rs.registration = r.id WHERE rs.subscription IN (SELECT s.id FROM subscriptions AS s WHERE s.recipient = $1)",
    [userId]
  );
